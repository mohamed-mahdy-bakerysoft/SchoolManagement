...
exports.getAllFeesRecords = async (req, res) => {
  try {
    const feesRecords = await FeesHistory.find().populate("studentId", "name studentId");
    res.status(200).json(feesRecords);
  } catch (error) {
    res.status(500).json({ message: "Failed to fetch fees records", error: error.message });
  }
};

// Create a new fees record
exports.createFeesRecord = async (req, res) => {
    const { transactionId, studentId, feeType, amount, paymentDate, status, remarks } = req.body;
    ...
    try {
      ...
      const feesRecord = new FeesHistory({ transactionId, studentId, feeType, amount, paymentDate, status, remarks });
...
exports.getFeesRecordByTransactionId = async (req, res) => {
    ...
    try {
      const feesRecord = await FeesHistory.findOne({ transactionId }).populate("studentId", "name studentId");
      if (!feesRecord) {
        return res.status(404).json({ message: "Fees record not found" });
      }
      res.status(200).json(feesRecord);
    } catch (error) {
      res.status(500).json({ message: "Failed to fetch fees record", error: error.message });
    }
  };
...
exports.updateFeesRecord = async (req, res) => {
    ...
    try {
      ...
      if (updates.studentId) {
        const studentExists = await Student.findOne({ studentId: updates.studentId });
        if (!studentExists) {
          return res.status(404).json({ message: "Student not found" });
        }
      }
      ...
      res.status(500).json({ message: "Failed to update fees record", error: error.message });
    }
  };
  ...
  exports.deleteFeesRecord = async (req, res) => {
    ...
    try {
      ...
      res.status(500).json({ message: "Failed to delete fees record", error: error.message });
    }
  };