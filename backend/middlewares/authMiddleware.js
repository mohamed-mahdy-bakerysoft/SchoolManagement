...
const verifyAll = async (req, res, next) => {
  ...
  try {
    ...
    if (!user || !['admin', 'Office Staff', 'Librarian'].includes(user.role)) {
      return res.status(403).json({ message: 'Access denied, valid privileges required' });
    }
...
module.exports = { verifyAdmin, verifyStaff, verifyLibrarian, verifyAdminOrStaff,verifyAll };
